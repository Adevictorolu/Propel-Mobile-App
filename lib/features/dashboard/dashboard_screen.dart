  ...
  Future<void> _loadDashboardData() async {
    ...
    try {
      if (profile.role == 'mentor') {
        final stats = await SupabaseService.fetchMentorDashboardStats(user.uid);
        final conns = await SupabaseService.fetchConnections(user.uid, 'mentor');
        setState(() {
          _stats = stats;
          _connections = conns;
        });
      } else {
        final stats = await SupabaseService.fetchMenteeDashboardStats(user.uid);
        final conns = await SupabaseService.fetchConnections(user.uid, 'mentee');
        List<MatchResult> recs = [];
        if (authProvider.menteeProfile != null) {
          recs = await SupabaseService.fetchRecommendedMentors(
