...
class _MentorOnboardingScreenState extends State<MentorOnboardingScreen> {
  ...
  void _addWorkHistory() {
    setState(() {
      _workHistory.add(WorkHistoryEntry(role: 'Software Developer', company: 'Acme Inc', years: 2));
    });
  }
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_expertiseTags.isEmpty) {
      ToastOverlay.show(context, 'Please add at least one expertise tag', type: ToastType.error);
      return;
    }
    ...
    try {
      await SupabaseService.completeMentorOnboarding(
        userId: user.id,
                ...
                children: [
                  ...
                  AppCard(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        const Text('Primary Area of Mentorship', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                        const SizedBox(height: 6),
                        DropdownButtonFormField<String>(
                          value: _areaOfMentorship,
                          items: AppConstants.availableMentorshipAreas
                              .map((a) => DropdownMenuItem(value: a, child: Text(a)))
                              .toList(),
                          onChanged: (v) => setState(() => _areaOfMentorship = v!),
                          decoration: const InputDecoration(contentPadding: EdgeInsets.symmetric(horizontal: 16, vertical: 12)),
                        ),
                        const SizedBox(height: 16),
                        AppTextField(
                          ...
                          validator: (v) => AppValidators.validateRequired(v, 'Years of experience'),
                        ),
                        const SizedBox(height: 16),
                        AppTextField(
                        ...
                        AppTextField(
                          label: 'Bio',
                          hintText: 'Describe your professional background and passions...',
                  ...
                  AppCard(
                    child: Column(
                      crossAxisAlignment: CrossAlignment.start,
                      children: [
                        const Text('Expertise Tags', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                          ...
                          children: [
                            Expanded(
                              child: TextField(
                                ...
                                decoration: const InputDecoration(hintText: 'Add a tag (e.g. React, System Design)'),
                        ...
                        Wrap(
                          ...
                          children: _expertiseTags.map((tag) => Chip(
                            ...
                            onDeleted: () => setState(() => _expertiseTags.remove(tag)),
                            backgroundColor: AppColors.brandGreen100,
                            labelStyle: const TextStyle(color: AppColors.brandGreen800, fontWeight: FontWeight.w600),
                          )).toList(),
                        ),
                      ],
                    ),
                  ...
                  AppCard(
                    child: Column(
                      crossAxisAlignment: CrossAlignment.start,
                      children: [
                        const Text('Mentorship Style', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                        const SizedBox(height: 6),
                        DropdownButtonFormField<String>(
                          value: _mentorshipStyle,
                          items: AppConstants.mentorshipStyles
                              .map((s) => DropdownMenuItem(value: s, child: Text(s)))
                              .toList(),
                          onChanged: (v) => setState(() => _mentorshipStyle = v!),
                          decoration: const InputDecoration(contentPadding: EdgeInsets.symmetric(horizontal: 16, vertical: 12)),
                        ),
                        const SizedBox(height: 16),
                        Text('Max Mentee Capacity: $_maxCapacity mentees', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                        Slider(
