...
class FirebaseService {
  ...
  static Future<void> completeMentorOnboarding({
    ...
    await saveMentorProfile(mProfile);
  }
  ...
  static Future<void> completeMenteeOnboarding({
    ...
    await saveMenteeProfile(meProfile);
  }
  ...
  static Future<List<Map<String, dynamic>>> fetchMentors({
    ...
    final docs = await _db.collection('profiles').where('role', isEqualTo: 'mentor').get();
  ...
  static Future<void> updateConnectionStatus(String connectionId, String status) async {
    await _db.collection('connections').doc(connectionId).update({'status': status});
  }
  ...
  static Future<Curriculum?> fetchCurriculum(String connectionId) async {
    final doc = await _db.collection('curricula').doc(connectionId).get();
    if (!doc.exists || doc.data() == null) return null;
  ...
  static Future<Curriculum> createCurriculum(String connectionId) async {
    ...
    await _db.collection('curricula').doc(connectionId).set(payload);
  ...
  static Future<Curriculum> addGoalToCurriculum(
    ...
    await _db.collection('curricula').doc(curriculumId).update({
  ...
  static Future<List<AppNotification>> fetchNotifications(String userId) async {
    final snapshot = await _db.collection('notifications').where('user_id', isEqualTo: userId).get();
  ...
  static Future<int> fetchUnreadNotificationCount(String userId) async {
    final snapshot = await _db.collection('notifications').where('user_id', isEqualTo: userId).where('is_read', isEqualTo: false).get();
  ...
  static Future<void> markAllNotificationsRead(String userId) async {
    final docs = await _db.collection('notifications').where('user_id', isEqualTo: userId).get();
